        ...
        <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
            ...
            <div className="grid lg:grid-cols-2 gap-12">
                ...
                <div className="space-y-4">
                    <div className="aspect-[4/3] rounded-[2rem] overflow-hidden border border-border shadow-md bg-muted">
                        <img
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                        {product.mediaUrls?.map((url, i) => (
                            <div key={i} className="w-24 h-24 rounded-2xl overflow-hidden border-2 border-transparent hover:border-primary cursor-pointer flex-shrink-0 transition-all">
                ...
                <div className="flex flex-col space-y-8">
                    <div className="space-y-4">
                        <div className="inline-flex gap-2">
                            <Badge variant="outline" className={`rounded-full px-4 py-1.5 ${product.status === 'active' ? 'bg-farm-green/10 text-farm-green' : 'bg-farm-amber/10 text-farm-earth'}`}>
                                {product.status === 'active' ? t("products.status.active") : t("products.status.sold")}
                            </Badge>
                            <Badge variant="secondary" className="rounded-full px-4 py-1.5 bg-muted/50">
                                {product.location}
                            </Badge>
                        </div>
                        ...
                        <div className="flex items-baseline gap-2">
                            ...
                            <span className="text-muted-foreground text-lg uppercase font-medium">{t("products.pricePerKg")}</span>
                        </div>
                    </div>
                    ...
                    <div className="pt-8 border-t border-border flex flex-col sm:flex-row gap-4 items-center sm:items-stretch">
                        <Button
                            ...
                            disabled={sendingInterest || product.status !== 'active' || currentUser?._id === product.userId}
                            className="flex-1 h-16 rounded-2xl text-lg font-bold gap-3 shadow-xl shadow-primary/20"
                        >
                            {sendingInterest ? <Loader2 className="w-5 h-5 animate-spin" /> : <MessageSquare className="w-5 h-5" />}
                            {t("products.interested")}
                        </Button>
                        <div className="flex gap-2">
                            <Button variant="outline" size="icon" className="w-16 h-16 rounded-2xl border-2 hover:bg-muted">
                    ...
                    <div className="mt-8 p-6 rounded-3xl border border-border bg-card shadow-sm flex items-center gap-4">
                        ...
                        <div className="flex-1">
                            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-0.5">Seller Information</p>
                            <h4 className="font-bold text-lg">{product.userName}</h4>
                            <p className="text-sm text-muted-foreground">Trusted seller in {product.location}</p>
                        </div>
